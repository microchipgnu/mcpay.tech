...
interface APITool { id: string; name: string; description: string; inputSchema: Record<string, unknown>; isMonetized: boolean; payment: Record<string, unknown> | null; status: string; createdAt: string; updatedAt: string }
interface MCPInputPropertySchema { type: string; description?: string;[key: string]: unknown }
interface MCPTool { name: string; description?: string; inputSchema: { type: string; properties: Record<string, MCPInputPropertySchema> }; annotations?: { title?: string; readOnlyHint?: boolean; destructiveHint?: boolean; idempotentHint?: boolean; openWorldHint?: boolean } }
export interface MCPServer { id: string; name: string; description: string; url: string; category: string; tools: MCPTool[]; icon: React.ReactNode; verified?: boolean }
interface APIServer { id: string; serverId: string; name: string; receiverAddress: string; description: string; metadata?: Record<string, unknown>; status: string; createdAt: string; updatedAt: string; tools: APITool[] }
...
export default function ServersPage() {
    ...
    // --- footer mode: "fixed" when short page, "static" when long page ---
    ...
    // fetch current page (API pagination)
    useEffect(() => {
    ...
    useEffect(() => {
        ...
        const settleThenMeasure = async () => {
            if (loading) {
                // keep it fixed during skeletons
                setFooterFixed(true)
                return
            }
            // wait for images + two RAFs so layout is stable
            await imgsLoaded()
        ...
        <div className="bg-background">
            <main>
                <div ref={contentRef} className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-8">
                    ...
                    {/* Pagination — appears only when long list, gives the visual gap above footer */}
                    {showPagination && (
                        <div className="mt-10">
                            <Pagination>
                                <PaginationContent>
            ...
            {/* Footer:
          - fixed at bottom ONLY when page is short (no overflow) → no scroll
          - normal flow with top gap when page is long */}
